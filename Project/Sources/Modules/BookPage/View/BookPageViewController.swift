...
// MARK: - Outlets
    ...
    @IBOutlet private weak var bookDatePickerHeightConstraint: NSLayoutConstraint?
...
// MARK: - Life Cycle
    override func viewDidLoad() {
        ...
        self.configureDatePickerConstraint()
    ...
    func configureTextField() {
        self.nameTextField?.attributedPlaceholder = NSAttributedString(string: translate("BookTitle"))
    ...
    func configureDatePickerConstraint() {
        self.bookDatePickerHeightConstraint?.constant = isSmallPhone ? 175 : 215
    }
    ...
    @IBAction func onBookPageActionButtonClicked(_ sender: Any) {
        if let writedName = self.nameBookTextField?.text {
            self.presenter?.onBookPageActionButtonClicked(pickedDate: self.bookDatePicker?.date ?? Date(),
...
extension BookPageViewController: BookPageViewProtocol {
    ...
    func show(data: BookPageData) {
        ...
        self.navItem?.title = data.isEditMode ? translate("EditInfo")  : translate("AddBook")
    }
    
    private func configureButton(_ isEditMode: Bool) {
        let titleLabel = isEditMode ? translate("Edit") : translate("Add")
    ...
    private func changeBookPageActionButtonEnabled (_ isEnabled: Bool) {
        ...
        self.bookPageActionButton?.backgroundColor = isEnabled ? UIColor.buttonBlue : UIColor.buttonGray
    }
